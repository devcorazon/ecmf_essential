...
//	printf("Initializing KTD2027 with I2C address: 0x%x\n", KTD2027_I2C_ADDR);
	...
	write_register(KTD2027_REG_EN_RST, 6);
	...
	switch (mode) {
		...
		case RGB_LED_MODE_ON:
			if (color == RGB_LED_COLOR_RED) {
				...
				write_all_register(0, 0, 0, 0, 0, 0, 0, 0, 159, 119, 0, 1, 1, 0);
		...
		case RGB_LED_MODE_NOT_CONF:
			switch(color) {
				case RGB_LED_COLOR_AQUA_RH:
					write_all_register(FLASH_PERIOD, 0, 0, 0, 0, 0, 0, 0, 0, 39, 39, 0, 1, 1);
					break;

				case RGB_LED_COLOR_GREEN_VOC:
					write_all_register(FLASH_PERIOD, 0, 0, 0, 0, 0, 0, 0, 0, 39, 0, 0, 1, 0);
					break;

				case RGB_LED_COLOR_YELLOW_LUX:
					write_all_register(FLASH_PERIOD, 0, 0, 0, 0, 0, 0, 0, 39, 29, 0, 1, 1, 0);
					break;
			}
			break;
